...
import { Camera, Video, Mic, StopCircle, RefreshCcw, PlayCircle, CircleX, Circle, Dot } from 'lucide-react';
    ...
    return (
        ...
        md:max-w-2xl md:aspect-video md:rounded-3xl bg-gray-900`}
            ...
            <div className="flex gap-10 items-center mt-12 bg-white/10 backdrop-blur-md p-8 rounded-full border border-white/20">
                <button onClick={onClose} className="p-4 bg-white/5 rounded-full text-white/50 hover:text-white transition-colors">
                    <CircleX className="w-8 h-8" />
                </button>

                {mode === 'img' && (
                    <button onClick={takePhoto} className="w-24 h-24 bg-white rounded-full border-[6px] border-blue-500 shadow-xl active:scale-90 transition-transform" />
                )}
                ...
                {(mode === 'img' || mode === 'video') && (
                    <button disabled={recording} onClick={toggleCamera} className={`p-4 bg-white/5 rounded-full text-white transition-colors ${recording ? 'opacity-20' : 'hover:bg-white/10'}`}>
